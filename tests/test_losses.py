...
class TestLoss:
    ...
    def test_calculate(self):
        ...
        loss_value = loss.calculate(y_true, y_true)
        assert loss_value == pytest.approx(0, 1.1e-7, 1.1e-7)
...
class TestCategoricalCrossentropy:
    ...
    def test_forward(self, y_true, raises):
        with raises:
            ...
            loss_value = loss.calculate(y_true, y_pred)
            assert loss_value == pytest.approx(0, 1.1e-7, 1.1e-7)  # y_pred values that are equal to
